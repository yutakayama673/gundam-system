...
export default function PartsInputList({
  ...
  const { handleAdd, handleRemove, checkEditPermission } = usePartsOperations(
    ...
    msNumber,
    partName,
    dbParts.length, // ← 初期部品数として渡す（必要なら）
    partTypeMap,
    (partName) => {
      setFunctions((prev) => {
        const updated = { ...prev };
  ...
  const handlePartNameClick = (index, partName) => {
    // DBから来た部品は編集不可
    if (dbParts.includes(partName)) return;
      ...
      {parts.map((part, index) => (
        <div key={index} className="part-input-row">
          ...
          <input
            ...
            value={part}
            placeholder="部品名"
            readOnly={dbParts.includes(part)}   // ← DBに存在する部品なら編集不可
          ...
          <div className="form-group">
            <button type="button" onClick={() => openMaterialDialog(index)}>
              素材選択
            </button>
          ...
          <button
            type="button"
            onClick={() => handleRemove(index, currentFunctions, currentDescriptions)}
          >
            － 削除
          </button>
        </div>
                ...
                <li key={option.code} style={{ marginBottom: "8px" }}>
                  <label style={{ display: "block" }}>
                    <input
                      type="checkbox"
                      checked={
                        (currentMaterials[parts[selectedIndex]] ?? []).includes(
                          option.label
                        )
                      }
                      onChange={() =>
                        toggleMaterial(parts[selectedIndex], option.label)
                      }
                    />

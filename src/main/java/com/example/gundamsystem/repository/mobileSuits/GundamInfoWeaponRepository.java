...
public interface GundamInfoWeaponRepository extends JpaRepository<GundamInfoWeapon, PartsId> {
    ...
    @Query("SELECT g FROM GundamInfoWeapon g WHERE g.mobileSuitNumber = :msNumber ORDER BY g.partsIndex")
    List<GundamInfoWeapon> findByMobileSuitNumberOrderByPartsIndex(@Param("msNumber") String msNumber);
    
    @Query("SELECT g FROM GundamInfoWeapon g WHERE g.mobileSuitNumber = :msNumber AND g.partsName = :partName ORDER BY g.partsIndex")
    GundamInfoWeapon findByMobileSuitNumberAndPartsNameOrderByPartsIndex(
        @Param("msNumber") String msNumber,
        @Param("partName") String partName
    );
    
    @Query("SELECT COALESCE(MAX(g.partsIndex), 0) " +
            "FROM GundamInfoWeapon g " +
            "WHERE g.mobileSuitNumber = :msNumber")
     int findMaxIndex(@Param("msNumber") String msNumber);
    
    @Modifying
    @Transactional
    @Query("DELETE FROM GundamInfoWeapon h WHERE h.mobileSuitNumber = :msNumber AND h.partsName = :partName")
    int deleteByMobileSuitNumberAndPartsName(@Param("msNumber") String msNumber, @Param("partName") String partName);
    
    @Query("SELECT h.partsId FROM GundamInfoWeapon h WHERE h.mobileSuitNumber = :msNumber AND h.partsName = :partName")
